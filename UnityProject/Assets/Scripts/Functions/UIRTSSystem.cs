// In UIRTSSystem.cs - Use the same approach as RTSSystem
// Replace the entire UIRTSSystem class with this:
...
public class UIRTSSystem : MonoBehaviour
{
    ...
    {
        if (Input.GetMouseButtonDown(0)) // REMOVE the ! before Input
        {
            StartSelection();
        }
        if (Input.GetMouseButton(0) && isSelecting) // REMOVE the ! before Input
        {
            UpdateSelectionBox();
        }
        if (Input.GetMouseButtonUp(0) && isSelecting) // REMOVE the ! before Input
        {
            EndSelection();
        }
    }
